...
- **Feature Docs**: `/home/tunc/Sync/Programs/ytmpd/docs/agent/like-indicator`
...
```
Relative path: docs/agent/like-indicator/STATUS.md
Absolute path: /home/tunc/Sync/Programs/ytmpd/docs/agent/like-indicator/STATUS.md
               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
|-- docs/
|   +-- agent/
|       +-- like-indicator/            <-- Your feature folder
...
Read `docs/agent/like-indicator/STATUS.md` to identify:
...
- Read `docs/agent/like-indicator/CODEBASE_CONTEXT.md`
...
- If you're on Phase 1, no previous summaries to read

**Location**: `docs/agent/like-indicator/summaries/`

### Step 3: Read Your Phase Plan

Open `docs/agent/like-indicator/PROJECT_PLAN.md` and read **ONLY** your phase section:
...
- Edit `docs/agent/like-indicator/CODEBASE_CONTEXT.md`
...
- **Output location**: `docs/agent/like-indicator/summaries/PHASE_XX_SUMMARY.md`
...
Edit `docs/agent/like-indicator/STATUS.md`:
...
Check the **Integrations** section in `docs/agent/like-indicator/STATUS.md`.
...
2. Commit with the message format: `Phase {N}: {brief description of what was built}`
   - Example: `Phase 1: Implement like indicator config and core sync/playlist logic`
...
**If Jira Issue is configured (not `disabled`):** Skip -- Jira is disabled for this feature.
...
### Activation
The project uses uv-managed virtualenv. Prefix commands with `uv run` or activate:
```bash
...
uv run python -m ytmpd

# Lint/format
uv run ruff check ytmpd/
uv run ruff format ytmpd/
```
...
- [ ] Read `docs/agent/like-indicator/STATUS.md` to identify your phase
- [ ] Read `docs/agent/like-indicator/CODEBASE_CONTEXT.md` for codebase knowledge
- [ ] Read the 2 most recent phase summaries from `docs/agent/like-indicator/summaries/`
- [ ] Read your phase section in `docs/agent/like-indicator/PROJECT_PLAN.md`
...
After completion:
- [ ] Update `docs/agent/like-indicator/CODEBASE_CONTEXT.md` with new discoveries and changes
- [ ] Create phase summary using the template
- [ ] Verify all completion criteria are met (or document why not)
- [ ] Update `docs/agent/like-indicator/STATUS.md`
...
1. Read `docs/agent/like-indicator/STATUS.md`
